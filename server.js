...
function generateSwastikAiMarketReport() {
    ...
    const gFutItem = (parsedLiveRates.allFutures || []).find(f => f.name.includes('GOLD'));
    if (gFutItem && gFutItem.buy > 0) goldMcxLive = gFutItem.buy;

    const sFutItem = (parsedLiveRates.allFutures || []).find(f => f.name.includes('SILVER'));
...
function parseRawSundhaTabStream(data) {
    ...
    lines.forEach(line => {
        ...
        if (parts.length >= 4) {
            ...
            if (!isFuture) {
                if (globalAdminSettings.isMasterHidden || globalAdminSettings.hiddenBuy[rawId]) finalBuy = 0;
                if (globalAdminSettings.isMasterHidden || globalAdminSettings.hiddenSell[rawId]) finalSell = 0;
            }
...
// 24/7 BACKGROUND SERVER SELF-KEEP-ALIVE HEARTBEAT ENGINE
setInterval(() => {
    http.get(`http://127.0.0.1:${PORT}/api/rates-json`, () => {}).on('error', () => {});
}, 10000);
...
server.listen(PORT, '0.0.0.0', () => {
    console.log(`Swastik Gold High-Speed Engine running at http://localhost:${PORT}/`);
});
